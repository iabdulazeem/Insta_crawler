...
class InstagramPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if item.get('_type') == "media":
            #print("dxm Media id : {}".format(item.get('media_id')))
    ...
    def _get_user_from_db(self, insta_uid):
        existing_user_query = f"select * from indow_apis_instagramprofile where insta_uid='{insta_uid}'"
        self.cursor.execute(existing_user_query)
        profile = self.cursor.fetchone()
        return profile

    def _fetch_current_media(self, insta_media_id):
        existing_media_query = f"select * from indow_apis_instagramusermedia where media_id='{insta_media_id}'"
        self.cursor.execute(existing_media_query)
        media = self.cursor.fetchone()
        return media

    def _update_spider_db_record(self):
        running_spider_query = f"update indow_apis_crawlerstats set status='Completed',user_scrapped='{self.insta_uid}',no_of_media_scrapped={self.media_count}  where unique_id='{self.crawler_id}';"
    ...
    def _parse_profile(self, item):
        query = f"insert into indow_apis_instagramprofile(insta_uid,is_active,created_at,last_updated_at,access_token,full_name,username,follows_count,folowed_by_count,media_count,is_business) values({item.get('insta_uid')},{True},'{time_now}','{time_now}','n/a','{item.get('full_name')}','{item.get('username')}',{item.get('follows_count')},{item.get('folowed_by_count')},{item.get('media_count')},{item.get('is_business')});"
    ...
    def _parse_media(self, item, insta_user_id):
        media_insight_id = 0
        self.cursor.execute("insert into indow_apis_instagrammediainsight(insta_user_id,is_active,created_at,last_updated_at,likes_count,comments_count,media_tags,media_caption,media_type,people_tagged,filter_used,post_created_time) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id;",
        ...
        try:
            print('dxm: new code')
            self.cursor.execute("insert into indow_apis_instagramusermedia(insta_user_id,is_active,created_at,last_updated_at,media_id,media_url,media_insight_id,crawler_id) values(%s,%s,%s,%s,%s,%s,%s,%s);",
                            (insta_user_id, True, time_now, time_now, item.get('media_id'), item.get('media_url'), media_insight_id, self.crawler_id))
            self.connection.commit()
        except Exception as e:
            #above query needs to be executed everytime, it can fail because of duplicate media, but
            #failure should not block other quiries that's why rollback is used
            self.connection.rollback()

    def _parse_comment(self, item):
        insta_media_id = item.get('media_id')
        media = self._fetch_current_media(insta_media_id)
        media_unique_id = media[3]
        self.cursor.execute("insert into indow_apis_instagrammediacomments(is_active,created_at,last_updated_at,comment_id,media_id,comment_text,comment_by) values(%s,%s,%s,%s,%s,%s,%s);",
